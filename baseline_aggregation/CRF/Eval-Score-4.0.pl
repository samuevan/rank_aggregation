...
my $fnFeature = $ARGV[0]; #arquivo de validacao ou teste
my $fnPrediction = $ARGV[1]; #arquivo de predicoes feitas pelo algoritmo a ser avaliado
my $fnResult = $ARGV[2]; #arquivo de saida
my $flag = $ARGV[3];
...
# changing the output format
    print FOUT "qid\tP\@1\tP\@2\tP\@3\tP\@4\tP\@5\tP\@6\tP\@7\tP\@8\tP\@9\tP\@10\tHITS\tMAP\n"; #ALTERACAO SAMUEL 
#---------------------------------------------
    my @prec;
    my $map = 0;
    my $total_hits = 0;
...
# output the real query id    	
        ...
        my $total_hits_q = $hsResult{$qid}{"total_hits"};
        if ($flag == 1)
        {
            print FOUT "$qid\t";
            ...
            print FOUT sprintf("%.4f\t", $total_hits_q);
    ...
    $map /= ($#qids + 1);
    print FOUT sprintf("%.4f\t", $total_hits);
    print FOUT sprintf("%.4f\n\n", $map);
...
sub EvalQuery
{
    ...
    my @qids = sort{$a <=> $b} keys(%$pHash); #me parece que somente ordena as queries pelo id
    for(my $i = 0; $i < @qids; $i ++)#para todas as queries
    {
        my $qid = $qids[$i];
        my @tmpDid = sort{$$pHash{$qid}{$a}{"lineNum"} <=> $$pHash{$qid}{$b}{"lineNum"}} keys(%{$$pHash{$qid}}); #ordena os documentos pela posicao da linha
        my @docids = sort{$$pHash{$qid}{$b}{"pred"} <=> $$pHash{$qid}{$a}{"pred"}} @tmpDid; #ordena(decrescente) documentos pelo score dado pelo algoritmo a ser avaliado. 
        my @rates;

        for(my $iPos = 0; $iPos < $#docids + 1; $iPos ++)
        {
            $rates[$iPos] = $$pHash{$qid}{$docids[$iPos]}{"label"};#armazena o label real dos documentos da query
        }

        my $map  = MAP(@rates);
        my @PAtN = PrecisionAtN($iMaxPosition, @rates);
        my $total_hits = num_hits(@rates); #ALTERADO SAMUEL
# modified by Jun Xu, calculate all possible positions' NDCG for MeanNDCG
        ...
        $hsResults{$qid}{"total_hits"} = $total_hits;
...
sub ReadInputFiles
{
    my ($fnFeature, $fnPred) = @_;  #recebe os parametros?
...
sub MAP
{
    #recebe os ratings ordenados por score e calcula o MAP a partir disso
    my @rates = @_;
...
sub num_hits
{
    ...
    my $numRelevant = 0;
    for(my $iPos = 0; $iPos < 9 + 1; $iPos ++) #$#rates + 1; $iPos ++)
    {
        if ($hsPrecisionRel{$rates[$iPos]} == 1)
        {
            $numRelevant ++;            
